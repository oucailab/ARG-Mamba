...
def get_img_padded(image, patch_size):
    ...
    pad = albu.PadIfNeeded(min_height=h, min_width=w, position='bottom_right', border_mode=0, value=[0, 0, 0])(image=image)
    return pad['image'], height_pad, width_pad
...
def make_dataset_for_one_huge_image(img_path, patch_size):
    ...
    for x in range(0, output_height, patch_size[0]):
        for y in range(0, output_width, patch_size[1]):
            tile_list.append(image_pad[x:x + patch_size[0], y:y + patch_size[1]])
...
def main():
    ...
    for img_path in img_paths:
        ...
        dataset, width_pad, height_pad, output_width, output_height, img_pad, img_shape = make_dataset_for_one_huge_image(img_path, patch_size)
        output_mask = np.zeros(shape=(output_height, output_width), dtype=np.uint8)
        output_tiles = []
        k = 0
        with torch.no_grad():
            dataloader = DataLoader(dataset=dataset, batch_size=args.batch_size, drop_last=False, shuffle=False)
