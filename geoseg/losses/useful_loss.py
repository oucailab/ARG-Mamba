...
class ABCNetLoss(nn.Module):
    ...
    def __init__(self, ignore_index=255):
        ...
        self.aux1_loss = FocalLoss(ignore_index=ignore_index)
        self.aux2_loss = FocalLoss(ignore_index=ignore_index)

    def forward(self, logits, labels):
        if self.training and len(logits) == 3:
            logit_main, logit_aux1, logit_aux2 = logits
            loss = 1.0*self.main_loss(logit_main, labels) + 1.0 * self.aux1_loss(logit_aux1, labels)+1.0 * self.aux2_loss(logit_aux2, labels)
        else:
            loss = self.main_loss(logits, labels)
        return loss
