...
class PotsdamDataset(Dataset):
    def __init__(self, data_root='/data1/jzl/RemoteSensing-Seg-master/RemoteSensing-Seg-master/data/potsdam/test', mode='val', img_dir='images_1024', mask_dir='masks_1024',
    ...
    def load_img_mask_dsm(self, index):
        ...
        try:
            ...
            dsm_array = dsm_array.astype(np.float32) / 255.0 # Ensure float32 and normalize to [0, 1]
...
# 添加用于检查尺寸匹配的调试函数
def debug_dsm_sizes(dataset_path='data/potsdam/train1gemi', num_samples=10):
    """检查DSM和RGB图像的尺寸是否匹配"""
...
# 添加用于检查空间对齐的辅助函数
def check_dsm_rgb_alignment(dataset_path='data/potsdam/train1gemi', num_samples=3):
    """验证DSM与RGB空间对齐的辅助函数"""
    dataset = PotsdamDataset(data_root=dataset_path, use_dsm=True)
